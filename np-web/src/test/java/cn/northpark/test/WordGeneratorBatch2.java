 ...
 */
@Slf4j
public class WordGeneratorBatch2 {
    public static void main(String[] args) {
        ...
        try {
            ...
            for (int i = 1; i <= personalSheet.getLastRowNum(); i++) {
                ...
                String id = getStringCellValue(row.getCell(4));//人员编号
                Map<String, String> personalInfo = new HashMap<>();
                for (int j = 0; j < fieldTitles1.length; j++) {
                    String fieldValue = getStringCellValue(row.getCell(j));
                    personalInfo.put(fieldTitles1[j], fieldValue);
                }
            ...
            for (int i = 1; i <= jobSheet.getLastRowNum(); i++) {
                ...
                String userId = getStringCellValue(row.getCell(2));
                Map<String, String> jobInfo = new HashMap<>();
                for (int j = 0; j < jobTitles1.length; j++) {
                    String fieldValue = getStringCellValue(row.getCell(j));
                    jobInfo.put(jobTitles1[j], fieldValue);
                }
            ...
            for (int i = 1; i <= memberSheet.getLastRowNum(); i++) {
                ...
                String userId = getStringCellValue(row.getCell(12));
                Map<String, String> memberInfo = new HashMap<>();
                for (int j = 0; j < memberTitles1.length; j++) {
                    String fieldValue = getStringCellValue(row.getCell(j));
                    memberInfo.put(memberTitles1[j], fieldValue);
                }
