...
//				 Elements last_a = parse.select("a");
//				 for(Element e:last_a){
//					 if(e.attr("href").contains("waitsun.com/?dl_id")){//改成短连接  并且改样式
...
//				String article = findByCondition.get(i).getArticle();
//				Document parse = Jsoup.parse(article);
//				parse.select("img").remove();
//				parse.select("strong").remove();
//				findByCondition.get(i).setArticle(parse.html());
...
//								
//								
//								LOGGER.info(tag222[j]+"      "+tagcode222[j]);
//								Tags model =  new Tags();
