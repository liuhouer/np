...
//		"classpath:application-transaction.xml", "classpath:application-mq.xml" })
...
//		// + "WHERE id IN ( SELECT id FROM ( SELECT max(id) AS id, count(moviename) AS
//		// count "
//		// + "FROM bc_movies GROUP BY moviename HAVING count > 1 ORDER BY count DESC )
//		// AS tab )";
//		//
