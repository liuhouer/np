    ...
    <div class="container">
        <div class="navbar-header">
            ...
            <div class="navbar-brand">
                ...
                <c:if test="${user==null }">href="/login" title="登陆" </c:if>
				 ...
				 <c:if test="${user==null  }">
				 	<img  src="/statics/img/davatar.jpg"  alt="davatar" class="img-circle max-width-50" height="40" width="40">
				 </c:if>
                ...
                &nbsp; &nbsp;
                <c:if test="${user!=null }"><a href="/cm/logout" title="退出" id="J_log_info_l"><span
                        class="glyphicon glyphicon-off"></span>退出</a></c:if>
                <c:if test="${user==null }"><a href="/login" title="登录" id="J_log_info_l"><span
                        class="glyphicon glyphicon-on"></span>登陆</a></c:if>
            </div>
        </div>
        <div class="navbar-collapse collapse mainhead-collapse">
            <ul class="nav mainhead-nav" id="J_tabs">
                ...
                <li cname="pcenter">
                    <a id="J_log_info_r" title="登录-个人中心" <c:if test="${user!=null }"> href="/cm/pcentral"</c:if><c:if
                            test="${user==null }">href="/login"</c:if>><c:if test="${user!=null }">我自己</c:if><c:if
                            test="${user==null }">登陆</c:if></a>
                </li>
            </ul>
        </div>
    </div>
