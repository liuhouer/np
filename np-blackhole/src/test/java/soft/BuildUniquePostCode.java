 ...
 */
@Slf4j
public class BuildUniquePostCode {
    public static void main(String[] args) {
        ...
        List<Map<String, Object>> mapList = NPQueryRunner.query("select id,ret_code,title,path from bc_soft where " +
                ...
                " or ret_code like '%：%'" +
                " or ret_code like '%,%' " +
                "or ret_code like '%post-%'  " +
                "or ret_code like '%-–-%'" +
                " or ret_code like '%!%' " +
                "or ret_code like '%——%'  )", new MapListHandler());
        for (int i = 0; i < mapList.size(); i++) {
            String ret_code = mapList.get(i).get("ret_code").toString();
            Object id = mapList.get(i).get("id");
            if(StringUtils.isNotBlank(ret_code)) {
                ret_code = ret_code.replace(":","").replace(".","-").replace(",","").replace("-–-","-").replace("!","").replace("——","-");
